...
Page({
  ...
  onShow() {
    ...
    // 检查是否需要刷新 (从加分页面返回)
    if (this.data.needRefresh && this.data.refreshRoomId) {
      console.log('检测到刷新请求，刷新房间数据', this.data.refreshRoomId);
      this.loadRoomData(this.data.refreshRoomId);
      // 重置刷新标记
      this.setData({
        needRefresh: false,
        refreshRoomId: ''
  ...
  startAutoRefresh() {
    ...
    if (this.data.isAutoRefreshEnabled && this.data.roomId) {
  ...
  // 切换自动刷新状态
  toggleAutoRefresh() {
    const newState = !this.data.isAutoRefreshEnabled;
    this.setData({ isAutoRefreshEnabled: newState });
  ...
  async loadRoomData(roomId) {
    ...
    // 处理从视图层传过来的事件对象
    if (roomId && roomId.currentTarget) {
      const eventRoomId = roomId.currentTarget.dataset.roomId;
      if (eventRoomId) {
        roomId = eventRoomId;
      } else {
    ...
    // 如果没有指定roomId，使用当前roomId
    if (!roomId) {
      roomId = this.data.roomId;
    }
    
    if (!roomId) {
      console.error('没有有效的房间ID');
      return;
    }
  ...
  showUserModal(e) {
    ...
    console.log('User score type:', typeof user.score, 'value:', user.score);
    
    // Create a copy of the user object with guaranteed score
    const userWithScore = {
      ...user,
      score: user.score !== undefined ? user.score : 0
    };
    
    this.setData({
      showUserModal: true,
      selectedUser: userWithScore
    });
  },
